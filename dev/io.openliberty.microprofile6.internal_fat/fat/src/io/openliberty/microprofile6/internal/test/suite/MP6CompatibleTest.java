...
public class MP6CompatibleTest {
    ...
    public void testMicroProfile60() throws Exception {
        try {
            server.setServerConfigurationFromFilePath("MP60.xml");
            server.startServer();
    ...
    public void testMP60andEE9() throws Exception {
        try {
            server.setServerConfigurationFromFilePath("MP60andEE9.xml");
            server.startServer();
            runGetMethod(200, "/helloworld/helloworld", MESSAGE);
        } finally {
            server.stopServer("CWMOT0010W"); //CWMOT0010W: OpenTracing cannot track JAX-RS requests because an OpentracingTracerFactory class was not provided or client libraries for tracing backend are not in the class path.
    ...
    public void testMP60andEE8() throws Exception {
        try {
            server.setServerConfigurationFromFilePath("MP60andEE8.xml");
