 ...
 *   'name': New name of the activation specification.
 *   'jndiName': New JNDI name for the activation specification.
 ...
 *   'alwaysActivateAllMDBs': MDB server-selection rule.  Determines which servers can drive MDBs deployed to them.  This parameter has two possible values: TRUE, FALSE.  Specify TRUE to always activate MDBs in all servers.  Otherwise, only servers with a running messaging engine are used.
 *   'retryInterval': The delay (in seconds) between attempts to connect to a messaging engine, both for the initial connection, and any subsequent attempts to establish a better connection.  Must be greater than zero.
 ...
 */
public class ModifySIBJMSActivationSpec extends Command {
	...
	private String forwarderDoesNotModifyPayloadAfterSet;
	private Boolean alwaysActivateAllMDBs;
	private Integer retryInterval;
	private Integer autoStopSequentialMessageFailure;
	private Long failingMessageDelay;

	public ModifySIBJMSActivationSpec(com.ibm.websphere.simplicity.ConfigIdentifier commandTarget) {
