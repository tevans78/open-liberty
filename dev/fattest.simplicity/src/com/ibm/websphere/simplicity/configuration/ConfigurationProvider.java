...
public abstract class ConfigurationProvider {
    ...
    /**
     * Creates a <code>ConfigurationProvider</code> instance using the specified config file. The
     * properties are read from the configruation file and loaded into memory.
     * 
     * @param configFile The configuration file to interact with
     * @return A <code>ConfigurationProvider</code> instance
     * @throws Exception
     */
	public static ConfigurationProvider getProvider(File configFile) throws Exception {
		Log.entering(c, "getProvider", new Object[] { configFile });
		ConfigurationProvider ret = null;
		
		if (configFile.exists()) {
			String name = configFile.getName();
			String ext = name.substring(name.lastIndexOf('.')+1);
			Log.debug(c, "Extension: "+ext);
			
			if (ext.equalsIgnoreCase("XML"))
				ret = new XmlConfigurationProvider(configFile);
			else if (ext.equalsIgnoreCase("props") || ext.equalsIgnoreCase("properties"))
				ret = new PropertiesConfigurationProvider(configFile);
		}
