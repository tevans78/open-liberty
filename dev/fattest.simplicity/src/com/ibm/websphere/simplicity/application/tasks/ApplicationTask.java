...
public abstract class ApplicationTask {
	...
	/**
	 * Sometimes the app task data requested during an update differs from the
	 * standard model.  Here we restrict the data to what is requested.
	 */
	public String[][] getTaskData(String[] columns) {
		if (taskData == null)
			return null;
		
		String[][] newTaskData = new String[taskData.length][columns.length];
		for (int i=0; i < columns.length; i++)
			newTaskData[0][i] = columns[i];
		
		for (int r=1; r < taskData.length; r++) {
			for (int c=0; c < columns.length; c++) {
				Integer oldcol = coltbl.get(columns[c]);
				newTaskData[r][c] = oldcol != null ? taskData[r][oldcol] : null;
			}
		}
		return newTaskData;
	}
