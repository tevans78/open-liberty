...
public class ApplicationOptions {
    ...
    public ApplicationOptions(List<ApplicationTask> tasks, Cell cell) throws Exception {
        this.tasks = tasks;
        this.cell = cell;
        if (tasks != null)
            loadTasks(tasks);
        ...
        if (modules == null) {
            ...
            if (mapModulesToServers != null)
                for (int i = 0; i < mapModulesToServers.size(); i++) {
                    ...
                    AssetModule module =
                        AssetModule.getModuleInstance(null, entry.getModule(), entry.getUri());
                    modules.add(module);
                }
        }
        return modules;
    ...
    /**
     * The ActSpecJNDI option binds Java 2 Connector (J2C) activation
     * specifications to destination Java Naming and Directory Interface (JNDI)
     * names. You can optionally bind J2C activation specifications in your
     * application or module to a destination JNDI name.
     */
     ...
     * specifications, auto-linking of JNDI names for EE5 apps, enabling and
     * disabling distribution of the app, and so on.
     */
    ...
    /**
     * Specify Java Naming and Directory (JNDI) name bindings for each
     * enterprise bean with a business interface in an EJB module. Each
     * enterprise bean with a business interface in an EJB module must be bound
     * to a JNDI name. For any business interface that does not provide a JNDI
     * name, or if its bean does not provide a JNDI name, a default binding name
     * is provided. If its bean provides a JNDI name, the default JNDI name for
     * the business interface is provided on top of its bean JNDI name by
     * appending the package-qualifed class name of the interface.
     * <p>
     * If you specify the JNDI name for a bean in the Provide JNDI names for
     * beans panel, Do not specify both the JNDI name and business interface
     * JNDI name for the same bean. If you do not specify the JNDI name for a
     * bean, you can optionally specify a business interface JNDI name. If you
     * do not specify a business interface JNDI name, the runtime provides a
     * container default.
     */
    ...
    /**
     * The BindJndiForEJBMessageBinding option Binds enterprise beans to
     * listener port names or Java Naming and Directory Interface (JNDI) names.
     * Ensure each message-driven enterprise bean in your application or module
     * is bound to a listener port name.
     */
     ...
     * Naming and Directory Interface (JNDI) names. Ensure each non
     * message-driven enterprise bean in your application or module is bound to
     * a JNDI name.
     */
    ...
    /**
     * The CorrectOracleIsolationLevel option specifies the isolation level for
     * the Oracle type provider. The last field of each entry specifies the
     * isolation level. Valid isolation level values are 2 or 4.
     */
    ...
    /**
     * The CorrectUseSystemIdentity option replaces RunAs System to RunAs Roles.
     * The enterprise beans that you install contain a RunAs system identity.
     * You can optionally change this identity to a RunAs role.
     */
    ...
    /**
     * The CtxRootForWebMod option edits the context root of the Web module. You
     * can edit a context root that is defined in the application.xml file using
     * this option.
     */
    ...
    /**
     * The CustomActivationPlan option specifies runtime components to add or remove 
     * from the default run time components that are used to run the application. 
     * Only use this option when the application server cannot obtain all necessary 
     * runtime components by inspecting the application.
     */
    public CustomActivationPlanTask getCustomActivationPlan() {
    	return customActivationPlan;
    }
     ...
     * specific data source to a CMP bean overrides the default data source for
     * the module that contains the enterprise bean.
     */
    ...
    /**
     * The DataSourceFor10EJBModules option specifies the default data source
     * for the enterprise bean module that contains 1.x container-managed
    ...
    /**
     * The DataSourceFor20EJBModules option specifies the default data source
     * for the enterprise bean 2.x module that contains 2.x container managed
     * persistence (CMP) beans.
     */
    ...
    /**
     * The EmbeddedRar option binds Java 2 Connector objects to JNDI names. You
     * must bind each Java 2 Connector object in your application or module,
     * such as, J2C connection factories, J2C activation specifications and J2C
     * administrative objects, to a JNDI name.
     */
     ...
     * unprotected methods of 1.x enterprise beans. Specify to leave the method
     * as unprotected, or assign protection which denies all access.
     */
     ...
     * role to the unprotected method, add the method to the exclude list, or
     * mark the method as cleared. You can assign multiple roles for a method by
     ...
     * beans. You must map each enterprise bean reference defined in your
     * application to an enterprise bean.
     */
    ...
    /**
     * The MapEnvEntryForEJBMod option edits the env-entry value of the EJB module. 
     * You can use this option to edit the value of environment entries in the EJB 
     * module, ejb-jar.xml.
     */
    ...
    /**
     * The MapEnvEntryForWebMod option edits the env-entry value of the Web module. 
     * You can use this to edit the value of env-entry values in the web.xml file. 
     */
    public MapEnvEntryForWebModTask getMapEnvEntryForWebMod() {
    	return mapEnvEntryForWebMod;
    }
    
    /**
     * The MapEnvEntryForApp option edits the env-entry value of the application. 
     * You can use this option to edit the value of env-entry in the application.xml 
     * file. 
     */
    public MapEnvEntryForAppTask getMapEnvEntryForApp() {
    	return mapEnvEntryForApp;
    }
    ...
    public MapEnvEntryForClientModTask getMapEnvEntryForClientMod() {
    	return mapEnvEntryForClientMod;
    }

    /**
     * The MapMessageDestinationRefToEJB option maps message destination
     * references to Java Naming and Directory Interface (JNDI) names of
     * administrative objects from the installed resource adapters. You must map
     * each message destination reference that is defined in your application to
    ...
    /**
     * The MapModulesToServers option specifies the application server where you
     * want to install modules that are contained in your application. You can
     * install modules on the same server, or disperse them among several
     * servers.
     */
    ...
    /**
     * The MapResEnvRefToRes option maps resource environment references to
     * resources. You must map each resource environment reference that is
     * defined in your application to a resource.
     */
    ...
    /**
     * The MapResRefToEJB option maps resource references to resources. You must
     * map each resource reference that is defined in your application to a
     * resource.
     */
    ...
    /**
     * The MapRolesToUsers option maps users to roles. You must map each role
     * that is defined in the application or module to a user or group from the
     * domain user registry. You can specify multiple users or groups for a
     * single role by separating them with a pipe (|).
     */
    ...
    /**
     * The MapRunAsRolesToUsers option maps RunAs roles to users. The enterprise
     * beans you that install contain predefined RunAs roles. Enterprise beans
     * that need to run as a particular role for recognition while interacting
     * with another enterprise bean use RunAs roles.
     */
    ...
    /**
     * The MapWebModToVH option selects virtual hosts for Web modules. Specify
     * the virtual host where you want to install the Web modules that are
     * contained in your application. You can install Web modules on the same
     * virtual host, or disperse them among several hosts.
     */
    ...
    /**
     * If your application contains EJB 3.0 or Web 2.5 modules, you can
     * optionally lock the deployment descriptor of one or more of the EJB 3.0
     * or Web 2.5 modules. If you set the metadata-complete attribute to true
     * and lock deployment descriptors, the product writes the complete module
     * deployment descriptor, including deployment information from annotations,
     * to XML format.
     * <p>
     * Annotations are a standard mechanism of adding metadata to Java classes.
     * You can use metadata to simplify development and deployment of Java EE 5
     * artifacts. Prior to the introduction of Java language annotations,
     * deployment descriptors were the standard mechanism used by Java EE
     * components. These deployment descriptors were mapped to XML format, which
     * facilitated their persistence. If you select to lock deployment
     * descriptors, the product merges Java EE 5 annotation-based metadata with
     * the XML-based existing deployment descriptor metadata and persists the
     * result.
     * 
     * @return
     */
    ...
    /**
     * To deploy Java-based Web services, you need an enterprise application,
     * also known as an EAR file that is configured and enabled for Web
     * services.
     * <p>
     * A Java™ API for XML-Based Web Services (JAX-WS) application does not
     * require additional bindings and deployment descriptors for deployment
     * whereas a Java API for XML-based RPC (JAX-RPC) Web services application
     * requires you to add additional bindings and deployment descriptors for
     * application deployment. JAX-WS is much more dynamic, and does not require
     * any of the static data generated by the deployment step required for
     * deploying JAX-RPC applications.
     * <p>
     * For JAX-WS Web services, the use of the webservices.xml deployment
     * descriptor is optional because you can use annotations to specify all of
     * the information that is contained within the deployment descriptor file.
     * You can use the deployment descriptor file to augment or override
     * existing JAX-WS annotations. Any information that you define in the
    ...
    /**
     * An application may specify the name of a JASPI authentication provider
     * and message-layer to be used for authenticating messages destined for all
     * Web modules in the application. Each Web module may also specify the name
     * of a different JASPI authentication provider and message-layer in order
     * to override the authentication provider and message-layer specified for
     * the application. The provider-name and message-layer specified for the
     * application will be used for all Web-modules in the application unless it
     * is overriden by specifying a different provider-name and message-layer
     * for any of the Web modules. Each provider-name specified must correspond
     * to the name of a Jaspi authentication provider that is defined in the
     * security configuration. Use the displayJaspiProviderNames command in the
     * administrative JaspiManagement command-group to display the names of all
     * defined JASPI authentication providers.
     */
    public MapJaspiProviderTask getMapJaspiProviderTask() {
        return mapJaspiProvider;
    }
    ...
    public String toTaskString() throws Exception {
        ...
        for (Field field : fields) {
            ...
            if (superclass.equals(ApplicationTask.class) || superclass
                .equals(MultiEntryApplicationTask.class)) {
    ...
    protected void loadTasks(List<ApplicationTask> tasks) throws Exception {
        for (ApplicationTask task : tasks) {
            if (task instanceof CustomTask) {
                ...
                if (!found)
                    Log.warning(c, "Task not found: " + task.getTaskName()
                        + "; make sure it's in the standardTasks map");
            }
        }
    }
