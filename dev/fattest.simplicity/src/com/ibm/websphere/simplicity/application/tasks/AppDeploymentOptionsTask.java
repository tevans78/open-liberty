...
import com.ibm.websphere.simplicity.application.types.ClientModeType;
...
public class AppDeploymentOptionsTask extends ApplicationTask {
    ...
    private boolean validateSchemaModified = false;
    private boolean clientModeModified = false;
    private boolean enableClientModuleModified = false;
    ...
    public void setValidateSchema(boolean value) {
    	validateSchemaModified = true;
    	setBoolean(AppConstants.APPDEPL_VALIDATE_SCHEMA, 1, value);
    }
    
    public boolean getValidateSchema() {
    	return getBoolean(AppConstants.APPDEPL_VALIDATE_SCHEMA, 1, AppConstants.APPDEPL_VALIDATE_SCHEMA_DEFAULT);
    }
    
    public boolean isValidateSchemaModified() {
    	return validateSchemaModified;
    }
    
    public void setClientMode(ClientModeType value) {
    	clientModeModified = true;
    	setItem(AppConstants.APPDEPL_CLIENT_MODE, 1, value.getValue());
    }
    
    public ClientModeType getClientMode() {
    	ClientModeType type = null;
		String s = getString(AppConstants.APPDEPL_CLIENT_MODE, 1);
		if (s != null)
			type = ClientModeType.valueOf(s);
		return type;
    }
    
    public boolean isClientModeModified() {
    	return clientModeModified;
    }
    
    public void enableClientModule() {
    	enableClientModuleModified = true;
    	setBoolean(AppConstants.APPDEPL_ENABLE_CLIENT_MODULE, 1, true);
    }
    
    public boolean getEnableClientModule() {
    	return getBoolean(AppConstants.APPDEPL_ENABLE_CLIENT_MODULE, 1);
    }
    
    public boolean isEnableClientModuleModified() {
    	return enableClientModuleModified;
    }
